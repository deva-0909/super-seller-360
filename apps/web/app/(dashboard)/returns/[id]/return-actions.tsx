...
export function ReturnActions({
  ...
  async function submitDisposition(e: React.FormEvent) {
    ...
    setLoading(true);
    setError(null);
    const { error } = await supabase.rpc("disposition_return", {
    ...
    return (
      <div>
        <p className="text-sm text-ink-muted">
          Waiting for the item to physically arrive.
        </p>
        <div className="mt-3">
    ...
    return (
      <form onSubmit={submitDisposition} className="flex flex-col gap-4">
        <p className="text-xs text-ink-muted">
          ...
          inspection result is &quot;Good&quot; — everything else leaves
          inventory untouched.
        </p>
        {!hasWarehouse ? (
  ...
  return <p className="text-sm text-ink-muted">This return is closed.</p>;
}
