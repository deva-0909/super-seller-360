...
export default async function ReturnDetailPage({
  ...
  const { data: ret } = await supabase
    .from("returns")
    .select(
      "return_id, status, return_reason, pickup_date, received_date, inspection_result, restock_status, refund_status, warehouse_id, orders(external_order_id, net_amount), warehouses(name)",
    )
    .eq("return_id", id)
    .single();
  ...
  const { data: movements } = await supabase
    ...
    .select("inventory_txn_id, quantity, before_qty, after_qty, created_at, products(name, sku)")
    .eq("reference_id", id)
    ...
    <div className="mx-auto max-w-2xl px-8 py-8">
      <div className="flex items-baseline justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-muted">
            {ret.return_reason ?? "No reason given"}
          </p>
        </div>
        <StatusPill
          status={
            ret.status === "restocked"
              ? "success"
              : ret.status === "quarantined" || ret.status === "claimed"
                ? "warning"
                : "neutral"
          }
        >
      ...
      <div className="mt-6 border border-line bg-surface p-5">
        <dl className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
          ...
          <dd className="font-data text-ink">
            {ret.received_date
              ? new Date(ret.received_date).toLocaleDateString()
              : "—"}
          </dd>
          <dt className="text-ink-muted">Inspection result</dt>
          <dd className="text-ink">{ret.inspection_result ?? "—"}</dd>
          <dt className="text-ink-muted">Restock status</dt>
          <dd className="text-ink">{ret.restock_status}</dd>
          <dt className="text-ink-muted">Refund status</dt>
          <dd className="text-ink">{ret.refund_status}</dd>
        </dl>
      </div>
